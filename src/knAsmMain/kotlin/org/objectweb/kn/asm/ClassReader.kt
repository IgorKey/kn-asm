    ...
    private fun readCode(mv: MethodVisitor, context: Context, u: Int) {
        ...
        while (u < codeEnd) {
            ...
            while (FRAMES && frame != null
                ...
                if (!zip || unzip) {
                    mv.visitFrame(
                        Opcodes.F_NEW, frame.localCount, frame.local!!,
                        frame.stackCount, frame.stack!!
                    )
                } else if (frame.offset != -1) {
                    mv.visitFrame(
                        frame.mode, frame.localDiff, frame.local!!,
                        frame.stackCount, frame.stack!!
                    )
                }
                if (frameCount > 0) {
            ...
            when (ClassWriter.TYPE[opcode].toInt()) {
                ...
                ClassWriter.WIDE_INSN -> {
                    opcode = b[u + 1].toInt() and 0xFF
                    if (opcode == Opcodes.IINC) {
                        mv.visitIincInsn(readUnsignedShort(u + 2), readShort(u + 4).toInt())
                        u += 6
                    } else {
                ...
                ClassWriter.VAR_INSN -> {
                    mv.visitVarInsn(opcode, b[u + 1].toInt() and 0xFF)
                    u += 2
                }
                ClassWriter.SBYTE_INSN -> {
                    mv.visitIntInsn(opcode, b[u + 1].toInt())
                    u += 2
                }
                ClassWriter.SHORT_INSN -> {
                    mv.visitIntInsn(opcode, readShort(u + 1).toInt())
                    u += 3
                }
                ClassWriter.LDC_INSN -> {
                    mv.visitLdcInsn(readConst(b[u + 1].toInt() and 0xFF, c)!!)
                    u += 2
                }
                ...
                ClassWriter.FIELDORMETH_INSN, ClassWriter.ITFMETH_INSN -> {
                    var cpIndex = items[readUnsignedShort(u + 1)]
                ...
                ClassWriter.IINC_INSN -> {
                    mv.visitIincInsn(b[u + 1].toInt() and 0xFF, b[u + 2].toInt())
                    u += 3
                }
                // case MANA_INSN:
                else -> {
                    mv.visitMultiANewArrayInsn(readClass(u + 1, c)!!, b[u + 3].toInt() and 0xFF)
                    u += 4
                }
            }

            // visit the instruction annotations, if any
            while (tanns != null && tann < tanns.size && ntoff <= offset) {
                if (ntoff == offset) {
                    val v = readAnnotationTarget(context, tanns[tann])
                ...
                ntoff = if (++tann >= tanns.size || readByte(tanns[tann]) < 0x43)
                    -1
                else
                    readUnsignedShort(tanns[tann] + 1)
            }
            while (itanns != null && itann < itanns.size && nitoff <= offset) {
                if (nitoff == offset) {
                    val v = readAnnotationTarget(context, itanns[itann])
                    readAnnotationValues(
                        v + 2, c, true,
                        mv.visitInsnAnnotation(
                            context.typeRef,
                ...
                nitoff = if (++itann >= itanns.size || readByte(itanns[itann]) < 0x43)
                    -1
                else
...
//                av.visit(name!!, Type.getType(readUTF8(v, buf)!!))
    ...
    fun readConst(item: Int, buf: CharArray?): Any? {
        ...
        when (b[index - 1].toInt()) {
            ...
            ClassWriter.STR -> return readUTF8(index, buf)
